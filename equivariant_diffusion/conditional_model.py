...
class ConditionalDDPM(EnVariationalDiffusion):
    ...
    def forward(self, ligand, pocket, return_info=False):
        ...
        # NOTE about centering: here, ligand and pocket positions are
        # in the COMPLEX CoM-free system
        ...
        # NOTE about centering: and here, they are in the LIGAND CoM-free system, which is
        # still the case after noising
        ...
        info = {
            "error_t_lig_pos": scatter_mean(
                squared_error[:, : self.n_dims], ligand["mask"], dim=0
            ).mean(),
            "error_t_lig_h": scatter_mean(
                squared_error[:, self.n_dims :], ligand["mask"], dim=0
            ).mean(),
        }
        ...
        if self.training:
            ...
            )  # no loss_x_pocket because we are in the fixed pocket setting
        ...
        info["eps_hat_lig_x"] = scatter_mean(
            net_out_lig[:, : self.n_dims].abs().mean(1), ligand["mask"], dim=0
        ).mean()
        info["eps_hat_lig_h"] = scatter_mean(
            net_out_lig[:, self.n_dims :].abs().mean(1), ligand["mask"], dim=0
        ).mean()
