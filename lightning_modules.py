...
class LigandPocketDDPM(pl.LightningModule):
    def __init__(
        ...
        smiles_list = (
            ...
            else np.load(os.path.join(self.datadir, eval_params.smiles_file))
        )
    ...
    def training_epoch_end(self, outputs) -> None:
        loss_pos_epoch = (
            torch.stack([x["error_t_lig_pos"] for x in outputs]).detach().mean()
        )
        loss_h_epoch = torch.stack([x["error_t_lig_h"] for x in outputs]).mean()
        self.log("epoch_loss_pos/train", loss_pos_epoch, on_epoch=True)
        self.log("epoch_loss_h/train", loss_h_epoch, on_epoch=True)
